...
class ReferenceTransform(object):
    ...
    def __call__(self, results):
        ...
        if random.random() > 0.8:
            ...
            results['rf_img'] = cv2.rotate(results['rf_img'], rotate)
        ...
        if self.keep_ratio:
            ...
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
        # img.shape: h, w, c 用来做roi align的
        results['rf_bbox'] = [0, 0, rf_img.shape[1], rf_img.shape[0]]
        
        rf_img = mmcv.imnormalize(rf_img, self.mean, self.std, self.to_rgb)  # normalize
        rf_img = mmcv.impad(rf_img, shape=self.img_scale)  # pad 往右下方padding
