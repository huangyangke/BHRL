...
def pytorch2onnx(config_path,
    ...
    if do_simplify:
        ...
        onnxsim.simplify(output_file, input_data=input_dic, custom_lib=ort_custom_op_path)
...
def parse_args():
    ...
    parser.add_argument('config', help='test config file path')# 模型配置文件路径
    parser.add_argument('checkpoint', help='checkpoint file')# onnx模型文件路径
    parser.add_argument('--input-img', type=str, help='Images for input')# 用于追踪和转化的输入图像路径
